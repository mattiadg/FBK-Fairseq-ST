...
def load_model_state(filename, model):
    ...
    def clean_encoder(model):
        return {k:v for k, v in model.items() if k != "encoder.version"}
    def clean_decoder(model):
        return {k:v for k, v in model.items() if k != "decoder.embed_positions._float_tensor"
                and k != "decoder.version"}
    #is_multilingual = any([key.startswith('models') for key in state['model'].keys()])
    has_encoder = any([key.startswith('encoder') for key in clean_encoder(state['model']).keys()])
    has_decoder = any([key.startswith('decoder') for key in clean_decoder(state['model']).keys()])
    ...
    if (not (has_encoder and has_decoder)) or (('modified' in state) and state['modified']):
