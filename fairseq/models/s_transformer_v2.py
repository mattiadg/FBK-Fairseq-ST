...
from fairseq.modules import (
    ...
    GradMultiply, MultiheadAttention, ConvMultiHeadAttention
)
...
@register_model('s_transformer_v2')
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, left_pad=True, convolutions=((512, 3),) * 20, stride=2,
        ...
        in_channels = 1
        for i, (out_channels, kernel_len, kernel_width) in enumerate(convolutions):
            kernel_size = (kernel_len, kernel_width)
            if isinstance(kernel_size, tuple):
                padding = ()
                for ks in kernel_size:
                    if ks % 2 == 1:
                        padding = padding + (ks // 2, )
                    else:
                        padding = padding + (0,)
            else:
                if kernel_size % 2 == 1:
            ...
            self.bn.append(BatchNorm(out_channels))
            if i == 0:
                stride = (min([4, kernel_size[0] - 1]), min([4, kernel_size[1] - 1]))
        self.pooling = nn.MaxPool2d(stride)

        self.relu = nn.ReLU()

        flat_dim = math.floor(audio_features / 4) * out_channels
        self.layers2d = nn.ModuleList([])
        self.layers1d = nn.ModuleList([])

        self.fc3 = Linear(flat_dim, args.encoder_output_dim)
        self.layers2d.extend([
            Transformer2DEncoderLayer(args)
            for _ in range(args.encoder_layers-1)
        ])
        self.layers1d.extend([
            TransformerEncoderLayer(args)
            for _ in range(args.encoder_layers+1)
        ])
        self.embed_positions = PositionalEmbeddingAudio(
            args.max_source_positions, args.encoder_output_dim, self.padding_idx,
            left_pad=left_pad,
            learned=args.encoder_learned_pos,
        ) if not args.no_token_positional_embeddings else None
        self.register_buffer('version', torch.Tensor([2]))

        self.normalize = True #args.encoder_normalize_before
        if self.normalize:
            self.layer_norm = nn.LayerNorm(args.encoder_output_dim)
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for i, conv in enumerate(self.convolutions):
            ...
            src_lengths = torch.floor(src_lengths.float() / 2).long()
            #if i == 0:
            #if i > 0:
            #    x = x + residual
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        super().__init__()
        self.embed_dim = args.encoder_output_dim
...
@register_model_architecture('s_transformer_v2', 's_transformer_v2')
...
@register_model_architecture('s_transformer_v2', 's_transformer_v2_paper')
def speechtransformer_fbk(args):
    ...
    args.encoder_output_dim = getattr(args, 'encoder_output_dim', 256)
    args.encoder_convolutions = getattr(args, 'encoder_convolutions', '[(16, 3, 3), (256, 3, 3)]')
    args.encoder_layers = getattr(args, 'encoder_layers', 3)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 768)
    args.encoder_mid_dim = getattr(args, 'encoder_mid_dim', 64)
    ...
    args.decoder_output_dim = getattr(args, 'decoder_out_embed_dim', 256)
